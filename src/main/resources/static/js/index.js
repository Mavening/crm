layui.use(['form','jquery','jquery_cookie'], function () {
    ...
    form.on("submit(login)",(data)=>{
        ...
        $.ajax({
            ...
            url: ctx+"/user/login",
            data: {
                //key的名字要与controller中保持一致
                //data.field的属性值就是前端的name值
                userName: data.field.username,
                userPwd: data.field.password
            },
            success: result => {
                //result是回调函数，用来接收后端返回的数据
                console.log(result);
                //判断是否登录成功，如果code=200，则表示成功，否则表示失败
                if(result.code==200){
                    //登录成功
                    //设置用户是登录状态
                    //1.利用session保存用户信息，会话存在为登录状态，否则为非登录状态。服务器关闭或浏览器关闭，会话销毁
                    //2.利用cookie保存用户信息，cookie未失效则用户是登录状态
