layui.use(['table','layer'],function(){
    ...
    function formatState(state) {
        if (state == 0) {
            return "<div class='layui-badge layui-bg-gray'>未分配</div>";
        } else if (state == 1) {
            return "<div class='layui-badge layui-bg-green'>已分配</div>";
        } else {
  ...
  /*  //监听数据表格的头部工具栏
    table.on('toolbar(saleChances)',(data)=>{
        ...
        if(data.event=='add'){
            //添加操作
            let title = '<h2>营销机会管理-添加营销机会</h2>';
            let context = ctx + '/saleChance/toSaleChancePage';
            //用layui.layer，这样最大化时不会覆盖父窗口
            layui.layer.open({
                //类型
                type: 2,
                //标题
...
*/
    ...
    function openSaleChanceDialog(saleChanceId) {
        ...
        var url = ctx + "/saleChance/toSaleChancePage";
